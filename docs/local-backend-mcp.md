...
- `.local/jlpt.sqlite` stores personal state: users, sessions, display settings, answers, review scheduling, mastery status, and future private notes.
...
Write-capable MCP tools remain draft-only. Promotion into official monthly files under `public/data/review-data/YYYY/MM.json` should require an explicit user review step.
