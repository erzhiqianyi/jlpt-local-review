...
6. Generate reading, listening, and mixed-practice material in the plan pack. Do not add unsupported item types to `public/data/review-data.json` until the website schema supports them.
7. Update `public/data/review-data.json` only when the user explicitly asks to import the generated material. Merge by `id`; do not replace existing user-created items or browser progress.
...
- Keep learner-specific scheduling state out of public seed data. The browser owns review counts, intervals, ease, and `nextReviewAt`.
